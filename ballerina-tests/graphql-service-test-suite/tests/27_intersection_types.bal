...
isolated function testIntersectionType(string jsonFileName, string operationName, json variables) returns error? {
    string document = check common:getGraphqlDocumentFromFile("intersection_types");
    string url = "http://localhost:9091/intersection_types";
    json actualPayload = check common:getJsonPayloadFromService(url, document, variables, operationName = operationName);
