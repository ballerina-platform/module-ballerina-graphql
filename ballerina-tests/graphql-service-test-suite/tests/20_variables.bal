...
isolated function testInvalidRequestWithVariables() returns error? {
    ...
    string actualPayload = check common:getTextPayloadFromBadRequest(url, request);
...
isolated function testInputVariables(string url, string resourceFileName, json variables, string? operationName = ()) returns error? {
    ...
    json actualPayload = check common:getJsonPayloadFromService(url, document, variables, operationName);
...
function dataProviderInputVariables() returns map<[string, string, json, string?]> {
    ...
    string url3 = "http://localhost:9090/special_types";
