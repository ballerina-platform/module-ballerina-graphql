...
isolated function testDecimalType1(string resourceFileName, json variables) returns error? {
    ...
    json actualPayload = check common:getJsonPayloadFromService(url, document, variables = variables);
    ...
    common:assertJsonValuesWithOrder(payloadWithFloatValues, expectedPayload);
}
