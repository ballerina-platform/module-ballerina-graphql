...
function testSingleFileUpload() returns error? {
    ...
    http:Client httpClient = check new ("http://localhost:9090", httpVersion = "1.1");
    ...
    string contentFromSource = check common:getContentFromByteStream(fileReadBlocksAsStream);
    common:assertJsonValuesWithOrder(receivedFileContent, contentFromSource);
}
...
function testMultipleFileUpload() returns error? {
    ...
    stream<io:Block, io:Error?> fileReadBlocksAsStream = check io:fileReadBlocksAsStream("./tests/resources/files/sample1.json");
    string jsonContentFromSource = check common:getContentFromByteStream(fileReadBlocksAsStream);
    common:assertJsonValuesWithOrder(receivedJsonFileContent, jsonContentFromSource);
    ...
    string txtContentFromSource = check common:getContentFromByteStream(fileReadBlocksAsStream);
    common:assertJsonValuesWithOrder(receivedTxtFileContent, txtContentFromSource);
    ...
    string xmlContentFromSource = check common:getContentFromByteStream(fileReadBlocksAsStream);
    common:assertJsonValuesWithOrder(receivedXmlFileContent, xmlContentFromSource);
}
