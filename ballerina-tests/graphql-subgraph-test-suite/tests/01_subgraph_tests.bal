...
function testQueryingSubgraph(string documentFileName) returns error? {
    ...
    json expectedPayload = check common:getJsonContentFromFile(documentFileName);
...
function testQueryingSubgraphWithVariables(string responseFileName, json variable) returns error? {
    string document = check common:getGraphqlDocumentFromFile("querying_subgraph_with_variable");
    string url = "localhost:9088/subgraph";
    json actualPayload = check common:getJsonPayloadFromService(url, document, variable);
    json expectedPayload = check common:getJsonContentFromFile(responseFileName);
    common:assertJsonValuesWithOrder(actualPayload, expectedPayload);
}
...
isolated function testQueringSdlOnSubgraph() returns error? {
    ...
    json expectedPayload = check common:getJsonContentFromFile("quering_sdl_on_subgraph");
    common:assertJsonValuesWithOrder(response, expectedPayload);
}
...
function testReferenceResolverReturningRecordWithObjectFields() returns error? {
    ...
    json response = check graphqlClient->execute(document);
    json expectedPayload = check common:getJsonContentFromFile("test_reference_resolver_returning_record_with_object_fields");
    common:assertJsonValuesWithOrder(response, expectedPayload);
}
