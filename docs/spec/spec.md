...
The generated GraphQL schema is exported as a Schema Definition Language(SDL) file by the GraphQL package. For the Ballerina projects, the schema file is written into `target` directory and for the single bal files, the schema is written into the same directory where the command is executed.
