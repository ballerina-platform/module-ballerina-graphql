...
**NOTE:** The generated GraphQL schema is exported by the GraphQL package in Schema Definition Language(SDL). For the Ballerina projects, the schema file is written into `target` directory and for the single bal files, the schema is written into the same directory where the bal file is located.
