...
isolated function getDocumentWithComments() returns string {
    string resourcePath = checkpanic getResourcePath();
    return readFileAndGetString(DOCUMENT_WITH_COMMENTS);
}

isolated function getDocumentWithParameters() returns string {
...
isolated function getDocumentWithTwoNamedOperations() returns string {
    string documentsDirPath = checkpanic getDocumentsPath();
    string path = checkpanic file:joinPath(documentsDirPath, DOCUMENT_TWO_NAMED_OPERATIONS);
    return readFileAndGetString(path);
}

isolated function getDocumentForResourcesWithRecord() returns string {
    string documentsDirPath = checkpanic getDocumentsPath();
    string path = checkpanic file:joinPath(documentsDirPath, DOCUMENT_QUERY_FOR_RESOURCES_WITH_RECORD);
    return readFileAndGetString(path);
}

isolated function getShorthandNotationDocument() returns string {
    string documentsDirPath = checkpanic getDocumentsPath();
...
isolated function getAnonymousOperationDocument() returns string {
    return readFileAndGetString(DOCUMENT_ANONYMOUS);
}

isolated function getNoCloseBraceDocument() returns string {
    return readFileAndGetString(DOCUMENT_NO_CLOSE_BRACE);
}

isolated function getTextWithQuoteFile() returns string {
    string textDirPath = checkpanic getTextPath();
    string path = checkpanic file:joinPath(textDirPath, TEXT_WITH_STRING);
    return readFileAndGetString(path);
}

isolated function getTextWithUnterminatedStringFile() returns string {
    string textDirPath = checkpanic getTextPath();
    string path = checkpanic file:joinPath(textDirPath, TEXT_WITH_UNTERMINATED_STRING);
    return readFileAndGetString(path);
}

isolated function getParsedJsonForDocumentWithParameters() returns json {
...
isolated function readJson(string path) returns json|error {
